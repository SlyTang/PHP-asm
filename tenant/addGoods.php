<!DOCTYPE html>
<html lang="en">
<head>
    <link rel="stylesheet" href="../CSS/customer.css" type="text/css">
    <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.6.3/css/all.css">

    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Add New Goods</title>
</head>
<body>

<div class="dropdwn">
    <nav>
        <ul>
        <li><a href="tenantMain.php">Home</a></li>
        <li><a href="#">Tenant<i class="fas fa-caret-down"></i> </a>
            <ul>
                <li><a href="manageGoods.php">Managed goods'</a></li>
                <li><a href="generateReport.php">Generate Report</a></li>
                <li><a href="deleteUserOrder.php">Delete user order</a></li>
            </ul>
            <li class = "logout"><a href="../login/logout.php">Logout</a></li>
        </li>
    </nav>
</div>

</head>
<body>
<div class = "mainContent">
    <div class="container">
        <div class="goodsData">
            <h2> ADD NEW GOODS </h2>
            <hr>
            <form action = "" method = "post">
                <div class="newGoods">
                    <label for = ""> Consignment Store ID </label>
                    <input type = "text" name = "storeID" class = "goods" placeholder = "Enter Consignment Store ID" require>
                </div>
                <div class="newGoods">
                    <label for = ""> Goods Name </label>
                    <input type = "text" name = "gName" class = "goods" placeholder = "Enter Goods Name" require>
                </div>
                <div class="newGoods">
                    <label for = ""> Stock Price </label>
                    <input type = "text" name = "sPrice" class = "goods" placeholder = "Enter Stock Price" require>
                </div>
                <div class="newGoods">
                    <label for = ""> Stock QTY </label>
                    <input type = "text" name = "qty" class = "goods" placeholder = "Enter Stock Price" require>
                </div>
                <div class="newGoods">
                    <label for = ""> Status</label>
                    <input type = "text" name = "status" class = "goods" placeholder = "1 = Available 2 = unavailable" require>
                </div>

                <button type = "submit" name = "insert" class = "btn btn-primary"> ADD </button>

                <a href = "manageGoods.php" class = "btn btn-danger"> CANCEL </a>
        </div>
    </div>
</body>
</html>

<?php
require_once "../connect/mysqli_conn.php";

if(isset($_POST['insert'])){

    $consignmentStoreID = $_POST['storeID'];
    $goodsName = $_POST['gName'];
    $stockPrice = $_POST['sPrice'];
    $remainingStock = $_POST['qty'];
    $status = $_POST['status'];

    $sql = "INSERT INTO goods(consignmentStoreID,goodsName,stockPrice,remainingStock,status)VALUES(
        '{$consignmentStoreID}',
        '{$goodsName}',
        '{$stockPrice}',
        '{$remainingStock}',
        '{$status}'
    )";
    $rs=mysqli_query($conn,$sql);

    if($rs){
        echo'<script> alert("Data Saved");</script>';
    }else{
        echo'<script> alert("Data Not Saved");</script>';
    }
}
?>